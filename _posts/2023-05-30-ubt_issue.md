...
然而，`*.pb.cc` 的缓存依赖头文件变量 `CachedIncludePaths` 为 null，因为在 `CachePCHUsageForModuleSourceFiles` 中只会给扩展名为 `.cpp` 的源文件赋值该变量。
...
目前，已经将问题反馈给负责维护 UBT 的同学，并共同讨论了几个解决方案，尚未得出最终结论。
